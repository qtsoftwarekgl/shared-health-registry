...
public class rhies_MedicationRequestResourceProvider implements IResourceProvider {

   private static final Logger LOGGER = LoggerFactory.getLogger(rhies_MedicationRequestResourceProvider.class);

   FhirContext ctx = FhirContext.forR4();
   IParser parser = ctx.newJsonParser();
   public rhies_MedicationRequestResourceProvider() {
      try {
   ...
   public Encounter read(@IdParam IdType theId) throws ResourceNotFoundException, SecurityException, IOException {
      ...
      DBCollection encounterCollection = CommonDbConnection.dbConnection().getCollection("medication_request");
      ...
      if (dbobject != null) {
         ...
         auditEvent.setOutcomeDesc("Medication search based on id: " + theId);
         rhies_FHIRBALPInterceptor.storeAuditEvent(auditEvent);
      } else {
         auditEvent.setOutcomeDesc("Medication search based on id: " + theId + " not found!");
         rhies_FHIRBALPInterceptor.storeAuditEvent(auditEvent);
   ...
   public MethodOutcome delete(@IdParam IdType theId) throws UnknownHostException, ResourceNotFoundException {
      ...
      DBCollection patientCollection = CommonDbConnection.dbConnection().getCollection("medication_request");
      BasicDBObject query = new BasicDBObject("id", theId.getValue().split("/")[1]);
      ...
      auditEvent.setRecorded(new Date());
      auditEvent.setOutcomeDesc(" medicationRequest deleted: " + theId.getValue().toString());
   ...
   public List<MedicationRequest> search(
      ...
      if (searchSet != null) {
         if (searchSet.getValue().equalsIgnoreCase(Constants.SEARCH_SET_MEDICATION_REQUEST)) {
            if (value != null) {
      ...
      DBCursor cursor = patientCollection.find(query).sort(new BasicDBObject("reasonCode.coding.display",1)).skip(pageNumber > 0 ? ((pageNumber - 1) * nPerPage) : 0)
              .limit(nPerPage);
      for (Iterator iterator = cursor.iterator(); iterator.hasNext(); ) {
         Object next = iterator.next();
         String patientData = next.toString();
         MedicationRequest patient = parser.parseResource(MedicationRequest.class, patientData);
      ...
      auditEvent.setType(new Coding().setSystem("http://hl7.org/fhir/restful-interaction").setCode("vread").setDisplay("search"));
      auditEvent.setRecorded(new Date());
      auditEvent.setOutcomeDesc( retVal.size() + " Medication Resource found");
   ...
   public MethodOutcome create(@ResourceParam String IncomingEncounter) throws NullPointerException, IOException {
      ...
      MedicationRequest medicationRequest = par.parseResource(MedicationRequest.class, IncomingEncounter);
      ...
      if (nida != null && !nida.trim().equals("")) { //nida exists
         ...
         DBObject dbobject = cursor.one();
         String encoded = par.encodeResourceToString(medicationRequest);
         ...
         if (dbobject != null) {
            ...
            auditEvent.setOutcomeDesc("Medication request Updated with value: " + value);
         } else {
            //NIDA not exists, check if not exists with PCID
            query = new BasicDBObject("id", medicationRequest.getId().split("/")[1]);
            cursor = patientCollection.find(query);

            dbobject = cursor.one();
            encoded = par.encodeResourceToString(medicationRequest);
            doc = (DBObject) JSON.parse(encoded);

            if (dbobject != null) {
               patientCollection.update(query, doc);
               auditEvent.setAction(AuditEvent.AuditEventAction.U);
               auditEvent.setOutcomeDesc("Medication Records Updated: id: " + medicationRequest.getId().split("/")[1]);
               auditEvent.setType(new Coding().setSystem("http://hl7.org/fhir/restful-interaction").setCode("update").setDisplay("update"));
            } else {
               patientCollection.insert(doc);
               auditEvent.setOutcomeDesc("Medication Records Created: id: " + medicationRequest.getId().split("/")[1]);
